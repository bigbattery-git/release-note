...
const summaryStatusLabels = {
  generated: "요약 생성",
  preserved: "기존 요약",
  not_applicable: "요약 원문 없음",
  failed: "요약 실패",
} as const;

const summaryStatusClasses = {
  generated: "bg-blue-50 text-blue-700",
  preserved: "bg-slate-100 text-slate-700",
  not_applicable: "bg-amber-50 text-amber-800",
    ...
    <li className="rounded-xl border border-slate-200 bg-white px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap justify-end gap-2">
          ...
          <span
            className={`rounded-full px-3 py-1 text-xs font-semibold ${summaryStatusClasses[item.summaryStatus]}`}
          >
            {summaryStatusLabels[item.summaryStatus]}
          </span>
        </div>
      </div>

      {item.summary ? (
        <p className="mt-4 whitespace-pre-wrap break-words border-t border-slate-100 pt-4 text-sm leading-7 text-slate-700">
          {item.summary}
        </p>
      ) : (
        <p
          className={`mt-4 border-t pt-4 text-sm ${item.summaryStatus === "failed" ? "border-red-100 text-red-700" : "border-slate-100 text-slate-500"}`}
        >
          {item.summaryError ?? "요약할 Release 원문이 없습니다."}
        </p>
      )}
    </li>
  );
